...
class Images(commands.Cog):
    ...
    @commands.command(brief="Get a pencil Sketch of your pfp",
                      usage="t!sketch [user (optional)]",
                      aliases=("psketch", "pencilsketch", "pencilate"))
    async def sketch(self, ctx, user: discord.Member=None):
        try:
            user = ctx.author if not user else user
            pfp = user.avatar_url.read()
            image = Image.open(BytesIO(await pfp))
            ...
            file = discord.File(buffer, filename="drawing.png")
            await ctx.send(f"Isn't my drawing superb? :3", file=file)
