...
class Utility(commands.Cog):
    ...
    async def on_message(self, message):
        async with self.db as conn:
            await conn.execute("""CREATE TABLE IF NOT EXISTS topmessages (
                userid bigint,
                messages integer,
                guild bigint)""")
            ...
            try:
                if conn:
                    check = await conn.fetchrow("SELECT userid FROM topmessages WHERE userid=$1", message.author.id)
                    if not check:
                        await conn.execute("INSERT INTO topmessages(userid, guild, messages) VALUES($1, $2, 1)",
                                           message.author.id, message.guild.id)
                    
                    else:
                        guild = await conn.fetch("SELECT guild FROM topmessages WHERE userid=$1", message.author.id)
                        guild = [list(i.values()) for i in guild]
                        msg_count = await conn.fetchrow("SELECT messages FROM topmessages WHERE userid=$1 AND guild=$2",
                                                        message.author.id, message.guild.id)
                        if msg_count:
                        ...
                        if [message.guild.id] in guild:
                            await conn.execute("UPDATE topmessages SET messages=$3 WHERE userid=$1 AND guild=$2",
    ...
    async def top(self, ctx, user: discord.Member=None):
        async with self.db as conn:
            await conn.execute("""CREATE TABLE IF NOT EXISTS topmessages (
                ...
                guild bigint)""")
            if not user:
                msgs = await conn.fetch("SELECT userid, messages FROM topmessages WHERE guild=$1", ctx.guild.id)
                msgs = set(msgs)
                m_count = dict()
                for i in msgs:
                ...
                for i in range(10):
                    try:
                        fin.append(f"{c}. {name[i]}{' '*(30-len(name[i]))} - {msg_count[i]}")
                        c += 1
                    except:
                        continue
                fin = "\n".join(fin)
                if fin:
                    await ctx.send(f"```css\n   Name{' '*26}Message count\n{fin}```")
                else:
                    await ctx.send("No messages yet")
            else:
                msgs = await conn.fetchrow("SELECT messages FROM topmessages WHERE guild=$1 AND userid=$2",
                                        ctx.guild.id, user.id)
                msgs = list(msgs.values())[0] if msgs else 0

                if not msgs:
                    await ctx.send("User doesn't have any messages yet")
                    return
                await ctx.send(f"{user.name}'s message count is {msgs}")
        ...
        try:
            async with self.db as conn:
                ...
                fig = go.Figure(go.Bar(y=name,
                                       x=msg_count,
                                       orientation='h',
                                       name=f"Top messages in {ctx.guild.name}",
                                       marker=dict(
